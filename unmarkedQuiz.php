<?php 
	...
	if(isset($_POST['updateMarks'])){
		...
		$query = "UPDATE submittedQuiz SET marks = ?, status = 'MARKED' WHERE id = ?";
	...
	$query = "SELECT submittedQuiz.id, submittedQuiz.name, submittedQuiz.dateSubmitted, submittedQuiz.fileUrl, students.fname, students.lname 
	          FROM submittedQuiz 
			  LEFT JOIN students 
			  ON submittedQuiz.studentId = students.id 
			  WHERE submittedQuiz.teacherId = ? AND submittedQuiz.status = 'UNMARKED' AND submittedQuiz.courseId = ? 
	...
	if($statement->num_rows){
		...
		$output = '<p>There are no unmarked quiz :)</p>';
	}
...
<div class = "container">
	<div class = "row" style = "margin-top:60px;">
		...
		<div class = "col-lg-9">
			<h3>UNMARKED QUIZ (<?php echo $statement->num_rows;?>)</h3> 
			<p><?php echo $msg;?> &nbsp;</p>
			<?php echo $output?>
		</div>
	</div>
